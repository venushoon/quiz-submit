...
function logout(){
  ...
  if (A.roomId) { A.roomId.disabled=false; A.roomId.focus(); }
  setOnline(false);
  buildStudentLink(false);
  saveLocal();
  // 탭을 문항으로
  activateTab(A.tabBuild);
}
...
function buildStudentLink(alsoQR=false){
  ...
  if(!roomId){ A.studentLink.value=""; if(alsoQR && A.qrCanvas){ const ctx=A.qrCanvas.getContext('2d'); ctx.clearRect(0,0,A.qrCanvas.width,A.qrCanvas.height);} return; }
...
(function init(){
  ...
  if(S_MODE){
    ...
    // 학생은 접속만 하고 대기 → 교사가 시작하면 문제 표시
    listenRoom(roomId); listenResponses(roomId);
    return;
  }

  // 관리자 모드는 '접속' 버튼으로만 연결(입력창 잠금은 connect에서 처리)
  if (A.roomId) A.roomId.disabled = false;
})();
