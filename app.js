...
// DOM 준비 유틸: 이미 로드되었으면 즉시, 아니면 load 후 실행
function ready(fn) {
...
async function connect() {
  ...
  if (els.sessionInput) els.sessionInput.disabled = true;
  ...
  if (els.sessionStatus) els.sessionStatus.textContent = `세션: ${ROOM} · 온라인`;
...
function disconnect() {
  ...
  if (els.sessionInput) els.sessionInput.disabled = false;
  ...
  if (els.sessionStatus) els.sessionStatus.textContent = `세션: - · 오프라인`;
  if (els.studentLink) els.studentLink.value = "";
  if (els.qrImg) els.qrImg.src = "";
  if (els.qList) els.qList.innerHTML = "";
  if (els.quizTitle) els.quizTitle.value = "";
}
...
function addQuestionUI() {
  ...
  const pushQ = (newQ) => {
    ...
    if (els.qAnswer) els.qAnswer.value = "";
    ["opt1", "opt2", "opt3", "opt4"].forEach(k => els[k] && (els[k].value = ""));
    if (els.qImg) els.qImg.value = "";
  };
...
async function submitStudent(answerPayload) {
  ...
  try {
    await window.db.runTransaction(async (tx) => {
      ...
      tx.update(roomRef, counterUpdate);

      if (r.policy?.openResult) setTimeout(() => alert(isCorrect ? "정답입니다! ✅" : "오답입니다. ❌"), 0);
      else setTimeout(() => alert("제출 완료!"), 0);
    });
  } catch (e) {
    console.error("제출 트랜잭션 실패:", e);
  }
}
...
function renderRoom(r) {
  ...
  if (els.pTitle) els.pTitle.textContent = r.title || "";
  if (els.sTitle) els.sTitle.textContent = r.title || "";
  if (els.chipJoin) els.chipJoin.textContent = r.counters?.join || 0;
  if (els.chipSubmit) els.chipSubmit.textContent = r.counters?.submit || 0;
  if (els.chipCorrect) els.chipCorrect.textContent = r.counters?.correct || 0;
  if (els.chipWrong) els.chipWrong.textContent = r.counters?.wrong || 0;

  const total = r.questions?.length || 0;
  const cur = r.currentIndex ?? -1;
  if (els.qCounter) els.qCounter.textContent = `Q${Math.max(0, cur + 1)}/${total}`;
  if (MODE === 'admin') {
    if (els.quizTitle) els.quizTitle.value = r.title || "";
    if (els.openResult) els.openResult.checked = !!r.policy?.openResult;
    if (els.brightMode) els.brightMode.checked = !!r.policy?.bright;
    if (els.timerSec) els.timerSec.value = r.policy?.timer || 30;
  ...
  if (MODE === 'admin') {
    ...
    if (r.mode === 'ended') {
      ...
      if (els.pQText) els.pQText.textContent = q.text || "";
  ...
  if (MODE === 'student') {
    ...
    if (r.mode === 'ended') {
      ...
      if (els.sState) els.sState.textContent = "";
      els.sDone && els.sDone.classList.remove("hide");
      const myBtn = els.btnMyResult || $('btnMyResult') || $('sMyResult'); // 이전 id 호환
      if (myBtn) myBtn.onclick = refreshMyResult;
      refreshMyResult();
    } else if (r.mode !== 'active' || !q) {
      if (els.sState) els.sState.textContent = "교사가 시작버튼을 누르면 퀴즈가 시작됩니다. 준비되었나요?";
      els.sQBox && els.sQBox.classList.add("hide");
    } else if (!r.accept) {
      if (els.sState) els.sState.textContent = r.revealed === cur ? "정답이 공개되었습니다." : "제출이 마감되었습니다. 다음 문항을 기다려주세요.";
      els.sQBox && els.sQBox.classList.add("hide");
    } else {
      if (els.sState) els.sState.textContent = "";
      els.sQBox && els.sQBox.classList.remove("hide");
      if (els.sQTitle) els.sQTitle.textContent = `Q${cur + 1}. ${q.text || ""}`;
      ...
      if (q.type === "mcq") {
        ...
        const shortSendBtn = els.btnShortSend || $('btnShortSend') || $('sShortSend'); // 이전 id 호환
        if (shortSendBtn) {
          shortSendBtn.onclick = () => {
            const val = (els.sShort?.value || "").trim();
            if (!val) { alert("정답을 입력하세요."); return; }
            submitStudent(val);
            shortSendBtn.disabled = true;
          };
        }
      }
    }
...
function renderQuestionList(questions = []) {
  ...
  allQuestions.forEach((q, index) => {
    ...
    if (isUnsaved) item.innerHTML += `<span class="chip" style="margin-left:auto; font-size:0.8em; padding:2px 6px;">저장 안됨</span>`;
...
function renderSubmitButton(chosen) {
  ...
  submitBtn.onclick = () => {
    ...
    else { submitStudent(chosen); submitBtn.disabled = true; }
  };
...
function updateTimer(roomData) {
  ...
  if (roomData.mode === 'active' && roomData.accept) {
    ...
    if (els.liveTimer) els.liveTimer.textContent = `${String(Math.floor(remaining / 60)).padStart(2, '0')}:${String(remaining % 60).padStart(2, '0')}`;
...
async function refreshMyResult() {
  ...
  if (!respSnap.exists) { if (els.myResult) els.myResult.innerHTML = "제출 기록이 없습니다."; return; }
  ...
  for (let i = 0; i < total; i++) {
    ...
    let ok = false, ansStr = "-";
    if (ans !== undefined) {
      if (q.type === "mcq") { ok = ans === q.answer; ansStr = q.options[ans] || "-"; }
      else { ok = String(ans || "").trim().toLowerCase() === String(q.answerText || "").trim().toLowerCase(); ansStr = String(ans); }
    }
    const correct = q.type === "mcq" ? q.options[q.answer] : q.answerText;
    html += `<tr><td>Q${i + 1}</td><td>${ansStr}</td><td>${correct}</td><td>${ans !== undefined ? (ok ? 'O' : 'X') : '-'}</td></tr>`;
  }
  html += `</tbody></table>`;
  if (els.myResult) els.myResult.innerHTML = html;
}
...
function listenForParticipants() {
  ...
  participantUnsub = window.FS.onSnapshot(responsesRef, (snapshot) => {
    const names = [];
    snapshot.forEach(doc => { const d = doc.data(); if (d?.name) names.push(d.name); });
    if (els.participantCount) els.participantCount.textContent = names.length;
    if (els.participantList) els.participantList.innerHTML = names.map(n => `<li>${n}</li>`).join('');
  });
}
...
function cacheDOMElements() {
  const ids = [
    ...
    'btnMyResult',           // 현재 버전 id
    'sShortSend', 'sMyResult' // 과거 버전 id 호환
  ];
  ids.forEach(id => { if (id === 'body') els.body = document.body; else els[id] = $(id); });
...
function init() {
  ...
  if (MODE === 'admin') {
    ...
    if (els.studentPanel) els.studentPanel.style.display = 'none';

    els.tabs.forEach(tab => on(tab, 'click', () => setTab(tab.id)));
    if (els.btnConnection) els.btnConnection.onclick = connect;
    if (els.btnBlank) els.btnBlank.onclick = makeBlank;
    if (els.btnSample) els.btnSample.onclick = loadSample;
    if (els.btnAddQ) els.btnAddQ.onclick = addQuestionUI;
    if (els.btnSaveQ) els.btnSaveQ.onclick = saveQuestions;
    if (els.btnResetQ) els.btnResetQ.onclick = resetQuestions;
    if (els.btnOptSave) els.btnOptSave.onclick = saveOptions;
    if (els.btnCopy) els.btnCopy.onclick = () => navigator.clipboard.writeText(els.studentLink?.value || "");
    if (els.btnOpen) els.btnOpen.onclick = () => { if (els.studentLink?.value) window.open(els.studentLink.value, "_blank"); };
    if (els.btnStart) els.btnStart.onclick = () => controlQuiz('start');
    if (els.btnPrev) els.btnPrev.onclick = () => controlQuiz('prev');
    if (els.btnNext) els.btnNext.onclick = () => controlQuiz('next');
    if (els.btnEnd) els.btnEnd.onclick = () => controlQuiz('end');
    if (els.btnReveal) els.btnReveal.onclick = () => controlQuiz('reveal');
    if (els.btnExport) els.btnExport.onclick = exportCSV;
    if (els.btnResetAll) els.btnResetAll.onclick = resetAll;
    if (els.btnToggleLink) els.btnToggleLink.onclick = () => {
      const hidden = els.studentLinkContainer?.classList?.toggle('hide');
      if (els.btnToggleLink) els.btnToggleLink.textContent = hidden ? '주소 보기' : '주소 숨기기';
    };
    ...
    if (els.studentPanel) els.studentPanel.style.display = 'block';
    if (els.btnJoin) els.btnJoin.onclick = joinStudent;
    ...
    if (ROOM) {
      ...
      window.FS.getDoc(docRef).then(snap => {
        ...
        if (snap.data().mode === 'ended') {
          ...
          els.sDone && els.sDone.classList.remove('hide');
          refreshMyResult();
          const myBtn = els.btnMyResult || $('btnMyResult') || $('sMyResult');
          if (myBtn) myBtn.onclick = refreshMyResult;
        } else {
