<!-- app.js : 아래 전체를 교체하여 저장 -->
<script type="module">
// ========= Firestore 모듈은 index.html에서 window.db / window.FS 로 바인딩됨 =========
// (index.html 최종본에서 그대로 가져옵니다)
...
function setMode(m){
  ...
  // 2) 패널(문항/옵션/프레젠테이션/결과)을 학생 모드에서 완전히 숨김
  [els.pBuild,els.pOptions,els.pPresent,els.pResults].forEach(p=>p?.classList.toggle("hide", !isAdmin));
  // 3) 학생 전용 루트
  els.studentAccess?.classList.toggle("hide", isAdmin);
  // 4) 기본 탭
  if(isAdmin) showTab("build");
}
...
function gatherBuilder(){
  ...
  const list=cards.map((c)=>{
    ...
    if(imgF) payload.image=URL.createObjectURL(imgF); // (미리보기용)
    if(type==='mcq'){
...
// 탭
[["tabBuild","build"],["tabOptions","options"],["tabPresent","present"],["tabResults","results"]].forEach(([k,tab])=>{
  els[k]?.addEventListener("click",()=> showTab(tab));
});
