...
const els = {
  ...
  builder: $("#builder"), btnUploadTxt: $("#btnUploadTxt"), fileUploadTxt: $("#fileUploadTxt"), btnDownloadTemplate: $("#btnDownloadTemplate"),

  // 옵션
  polDevice: $("#polDevice"), polName: $("#polName"), chkAccept: $("#chkAccept"), chkReveal: $("#chkReveal"), chkBright: $("#chkBright"),
  timerSec: $("#timerSec"), btnStartTimer: $("#btnStartTimer"), btnStopTimer: $("#btnStopTimer"),
  btnOptSave: $("#btnOptSave"), btnResetAll: $("#btnResetAll"),
...
function autoReconnect(){
  // URL role 우선 → 그다음 local
  const url = new URL(location.href);
  const role = url.searchParams.get("role");
  const room = url.searchParams.get("room");
  if(role) MODE = (role==="student"?"student":"admin");
  if(room) roomId = room;

  loadLocal();                 // local 보정
  setMode(MODE||"admin");
  if(roomId && MODE==="admin") { els.roomId.value=roomId; connect(); }
  if(roomId && MODE==="student") { buildStudentLink(); listenRoom(roomId); listenResponses(roomId); }
}
...
function gatherBuilder(){
  ...
  const list=cards.map((c)=>{
    ...
    if(imgF){ payload.image=URL.createObjectURL(imgF); } // 미리보기
    if(type==='mcq'){
...
els.btnOptSave?.addEventListener("click", async ()=>{
  if(!roomId) return alert("세션 접속 후 저장하세요.");
  const policy = els.polName?.checked ? "name" : "device";
  const timer  = parseInt(els.timerSec.value,10)||30;
  const bright = !!els.chkBright.checked;
  const accept = !!els.chkAccept.checked;
  const reveal = !!els.chkReveal.checked;
  await setDoc(roomRef(roomId), { policy, timer, bright, accept, reveal }, { merge:true });
  buildStudentLink();
  alert("옵션 저장 완료!");
});
els.btnResetAll?.addEventListener("click", async ()=>{
  if(!roomId) return;
  if(!confirm("문항/옵션/결과를 모두 초기화합니다. 진행할까요?")) return;
  await setDoc(roomRef(roomId), { title:"새 세션", mode:"idle", currentIndex:-1, accept:false, reveal:false,
    policy:"device", timer:30, bright:false, questions:[] }, { merge:true });
  // 응답들 삭제는 운영 시 Cloud Functions/보조 페이지 권장
  alert("초기화 완료!");
});

els.btnCopyLink?.addEventListener("click", ()=>{
  if(!els.studentLink.value) return;
  navigator.clipboard.writeText(els.studentLink.value).then(()=>alert("복사됨"));
});
els.btnOpenStudent?.addEventListener("click", ()=>{
  if(!els.studentLink.value) return;
  window.open(els.studentLink.value, "_blank","noopener,noreferrer");
});
...
els.btnStartTimer?.addEventListener("click", ()=>startTimer(parseInt(els.timerSec.value,10)||30));
...
els.btnShowMy?.addEventListener("click", ()=>{
  const r=window.__room||{}; const mine=(window.__resp||[]).find(x=>x.id===me.id);
  if(!mine){ els.myResult.innerHTML="제출 내역이 없습니다."; return; }
  const qs=r.questions||[]; let html="<ul>";
  qs.forEach((q,i)=>{ const a=mine.answers?.[i]; html+=`<li>Q${i+1}: ${a? (q.type==='mcq'? `선택 ${a.value+1}` : a.value) : '-'}</li>`; });
  html+="</ul>"; els.myResult.innerHTML=html;
});
