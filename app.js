/* =========================
   실시간 퀴즈 – 자동 재접속 · 타이머/오토다음 · 소형 UI 톤
   Firestore 연결 실패 시 localStorage로 폴백
   ========================= */
...
function bindElements(map){
  const out={}; for(const k in map){
    const v=map[k]; out[k]=v.startsWith('.')?Array.from(document.querySelectorAll(v)):document.getElementById(v);
  } return out;
}
...
let db=null, FS=null;
const ENV={useFS:false, subs:[], onRoom:null, onResponses:null};
const KEYS={
  LAST_ROOM:'quiz:last_room',
  LAST_MODE:'quiz:last_mode',
  DEVICE_TOKEN:'quiz:device_token',
  LS_ROOM:(id)=>`quiz:${id}`
};
...
(async function initFS(){
  try{
    ...
    if(window.firebase?.initializeApp){
      ...
      db=firebase.firestore(app); FS=firebase.firestore; ENV.useFS=true;
    }
  }catch{ /* 폴백 */ }
})();

// ---------- LocalStorage 모델 ----------
function lsRead(id){ const raw=localStorage.getItem(KEYS.LS_ROOM(id)); return raw?JSON.parse(raw):null; }
function lsWrite(id,data){ localStorage.setItem(KEYS.LS_ROOM(id), JSON.stringify(data)); }
function ensureRoomLocal(id){
  const cur=lsRead(id); if(cur) return cur;
  const room={ title:"새 세션", policy:"device", goldenBell:false, mode:"idle", accept:false, reveal:false,
               currentIndex:-1, timerEndAt:null, autoNext:false, questions:[], responses:{} };
  lsWrite(id,room); return room;
}
async function fsGetRoom(id){
  if(!ENV.useFS) return ensureRoomLocal(id);
  try{ const s=await db.collection('rooms').doc(id).get(); return s.exists?s.data():null; }
  catch{ ENV.useFS=false; return ensureRoomLocal(id); }
}
async function fsSetRoom(id,patch){
  if(!ENV.useFS){ const cur=ensureRoomLocal(id); lsWrite(id,{...cur,...patch}); return; }
  try{ await db.collection('rooms').doc(id).set(patch,{merge:true}); }
  catch{ ENV.useFS=false; return fsSetRoom(id,patch); }
}
async function fsGetResponses(id){
  if(!ENV.useFS){ const cur=ensureRoomLocal(id); return Object.entries(cur.responses||{}).map(([i,v])=>({id:i,...v})); }
  try{ const ss=await db.collection('rooms').doc(id).collection('responses').get(); const arr=[]; ss.forEach(d=>arr.push({id:d.id,...d.data()})); return arr; }
  catch{ ENV.useFS=false; return fsGetResponses(id); }
}
async function fsSetResponse(id,uid,patch){
  if(!ENV.useFS){ const cur=ensureRoomLocal(id); const u=cur.responses[uid]||{name:'',alive:true,answers:{}}; cur.responses[uid]=deepMerge(u,patch); lsWrite(id,cur); return; }
  try{ await db.collection('rooms').doc(id).collection('responses').doc(uid).set(patch,{merge:true}); }
  catch{ ENV.useFS=false; return fsSetResponse(id,uid,patch); }
}
...
function unsubscribeAll(){ ENV.subs.forEach(fn=>fn()); ENV.subs=[]; }
function listenRoom(id, handler){
  if(!ENV.useFS){ const t=setInterval(async()=>handler(await fsGetRoom(id)),800); ENV.subs.push(()=>clearInterval(t)); handler(ensureRoomLocal(id)); return; }
  const u=db.collection('rooms').doc(id).onSnapshot(s=>s.exists&&handler(s.data())); ENV.subs.push(u);
}
function listenResponses(id, handler){
  if(!ENV.useFS){ const t=setInterval(async()=>handler(await fsGetResponses(id)),800); ENV.subs.push(()=>clearInterval(t)); handler([]); return; }
  const u=db.collection('rooms').doc(id).collection('responses').onSnapshot(s=>{ const arr=[]; s.forEach(d=>arr.push({id:d.id,...d.data()})); handler(arr); }); ENV.subs.push(u);
}
...
async function connectRoom(){
  ...
  policy=r?.policy||'device'; goldenBell=!!r?.goldenBell;
  el.policyText.textContent=`정책: ${policy==='device'?'기기당 1회':'실명당 1회'} · 골든벨: ${goldenBell?'ON':'OFF'}`;
  unsubscribeAll(); listenRoom(roomId, renderRoom); listenResponses(roomId, renderResponses);
  await refreshStudentLink();
  el.statusText.textContent=`세션: ${roomId} · ${ENV.useFS?'온라인':'오프라인'}`;
}
...
async function drawQR(text){
  el.qrBox.innerHTML=''; try{
    if(window.QRCode?.toCanvas){ const canvas=document.createElement('canvas'); el.qrBox.appendChild(canvas);
      await new Promise((res,rej)=>QRCode.toCanvas(canvas,text,(e)=>e?rej(e):res())); return; }
  }catch{}
...
el.btnBuildForm.addEventListener('click',()=>{
  ...
  el.builder.innerHTML=''; for(let i=0;i<n;i++) el.builder.appendChild(buildRow(i+1));
});
el.btnLoadSample.addEventListener('click',()=>{
  ...
  el.quizTitle.value='샘플 퀴즈'; el.questionCount.value=samples.length; el.builder.innerHTML='';
  samples.forEach((q,i)=>el.builder.appendChild(buildRow(i+1,q)));
});
el.btnSaveQuiz.addEventListener('click', async ()=>{
  if(!roomId) return toast('세션부터 접속하세요.');
  const payload=collectQuiz(); if(payload.questions.length===0) return toast('문항이 없습니다.');
  policy=el.policySelect.value; goldenBell=!!el.goldenBellToggle.checked;
  await fsSetRoom(roomId,{ title:payload.title, policy, goldenBell,
    mode:'idle', accept:false, reveal:false, currentIndex:-1, timerEndAt:null, autoNext:!!el.autoNextToggle?.checked, questions:payload.questions });
  el.policyText.textContent=`정책: ${policy==='device'?'기기당 1회':'실명당 1회'} · 골든벨: ${goldenBell?'ON':'OFF'}`;
  toast('저장 완료');
});
el.btnExportJSON.addEventListener('click', async ()=>{
  if(!roomId) return; const r=await fsGetRoom(roomId);
  const blob=new Blob([JSON.stringify(r,null,2)],{type:'application/json'});
  const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`${r.title||roomId}.json`; a.click(); URL.revokeObjectURL(a.href);
});
el.btnImportJSON.addEventListener('click',()=>el.fileImport.click());
el.fileImport.addEventListener('change', async e=>{
  const f=e.target.files?.[0]; if(!f) return; const text=await f.text();
  try{ const data=JSON.parse(text); if(!roomId){ roomId=prompt('세션 코드 입력')||''; if(!roomId) return; }
    await fsSetRoom(roomId,data); toast('불러오기 완료'); } catch{ toast('JSON 형식 오류'); } e.target.value='';
});

function buildRow(no,q){
  ...
  radios.forEach(r=>r.addEventListener('change',()=>{
    ...
    mcq.classList.toggle('hidden',isShort); short.classList.toggle('hidden',!isShort);
  }));
  return wrap;
}
function collectQuiz(){
  ...
  const questions=cards.map((card,idx)=>{
    ...
    if(type==='mcq'){ const opts=$$('.opt',card).map(x=>x.value.trim()).filter(Boolean);
    ...
    } else { const answerText=$('.ansText',card).value.trim(); return {type:'short',text,answerText}; }
  }).filter(Boolean);
...
el.btnTimerStart.addEventListener('click', async ()=>{
  const sec = clamp(parseInt(el.timerSec.value||'30',10),5,600);
  await updateRoom({ timerEndAt: Date.now()+sec*1000, accept:true, reveal:false });
});
el.btnTimerStop.addEventListener('click', ()=>updateRoom({ timerEndAt:null }));

async function updateRoom(patch){ if(!roomId) return; await fsSetRoom(roomId, patch); }
async function stepIndex(d){
  const r=await fsGetRoom(roomId); const next=clamp((r.currentIndex??-1)+d,0,Math.max(0,(r.questions?.length||1)-1));
  await updateRoom({ currentIndex:next, accept:true, reveal:false, timerEndAt:null });
}

// ---------- 렌더링 ----------
let uiTicker=null;
function clearTicker(){ if(uiTicker) clearInterval(uiTicker); uiTicker=null; }
...
async function renderRoom(r){
  ...
  el.toggleAccept.checked=!!r.accept; el.toggleReveal.checked=!!r.reveal;
  el.autoNextToggle.checked=!!r.autoNext;

  // 타이머 표시(교사/학생 공통)
  clearTicker();
  const tick=()=>{
    const left=Math.max(0,(r.timerEndAt||0)-Date.now());
    const mm=Math.floor(left/60000), ss=Math.floor((left%60000)/1000);
    const fmt=(n)=>String(n).padStart(2,'0');
    el.remainTime.textContent=`${fmt(mm)}:${fmt(ss)}`;
    el.sRemain.textContent=`${fmt(mm)}:${fmt(ss)}`;
    // 타임업 처리(교사화면에서만 트리거)
    if(MODE==='teacher' && r.timerEndAt && left<=0){
      clearTicker();
      // 결과 공개하고 허용 종료
      updateRoom({ accept:false, reveal:true, timerEndAt:null });
      if(r.autoNext){ setTimeout(()=>stepIndex(1), 800); }
    }
  };
  tick(); uiTicker=setInterval(tick,250);
  ...
  if(MODE==='student'){
    ...
    if(r.mode!=='active' || !q){
      el.quizTypeBadge.textContent='대기'; el.questionText.textContent='대기 중입니다…';
      el.progressText.textContent='0 / 0'; el.optionsContainer.innerHTML=''; el.subjectiveBox.classList.add('hidden'); el.btnSubmitMCQ.classList.add('hidden'); return;
    }
    el.progressText.textContent=`${idx+1} / ${r.questions.length}`;
    el.quizTypeBadge.textContent=q.type==='mcq'?'객관식':'주관식'; el.questionText.textContent=q.text;
    if(q.type==='mcq'){ el.subjectiveBox.classList.add('hidden'); el.btnSubmitMCQ.classList.remove('hidden'); renderMCQ(q,r.accept,r.reveal); }
...
function renderMCQ(q, accepting, revealed){
  ...
  q.options.forEach((opt,i)=>{
    const b=document.createElement('button'); b.className='option'; b.textContent=opt; b.dataset.opt=String(i);
...
async function renderResponses(arr){
  ...
  arr.forEach(s=>{
    const a=s.answers?.[r.currentIndex]; const tag=document.createElement('div'); tag.className='chip'; tag.textContent=s.name||s.id;
    if(s.alive===false){ tag.classList.add('bad'); out++; }
  ...
  if(MODE==='teacher' && q && q.type==='short'){
    el.shortAnswers.innerHTML=''; arr.forEach(s=>{
      const a=s.answers?.[r.currentIndex]; if(!a || typeof a.value!=='string') return;
...
el.btnJoin.addEventListener('click', async ()=>{
  ...
  const uid = (policy==='device') ? getDeviceToken() : name; me={id:uid,name};
  await fsSetResponse(roomId, uid, { name, joinedAt:Date.now(), alive:true });
  toast(`${name} 님, 참가 완료!`);
});
function getDeviceToken(){ let t=localStorage.getItem(KEYS.DEVICE_TOKEN); if(!t){ t=id(); localStorage.setItem(KEYS.DEVICE_TOKEN,t);} return t; }

el.btnSubmitSubjective.addEventListener('click', async ()=>{
  const r=window.__room; if(!r) return; if(!r.accept) return toast('지금은 제출할 수 없습니다.');
...
el.btnSubmitMCQ.addEventListener('click', async ()=>{
  const r=window.__room; if(!r) return; if(!r.accept) return toast('지금은 제출할 수 없습니다.');
  const chosen=window.__myAnswer?.idx; if(typeof chosen!=='number') return toast('보기 하나를 선택하세요.');
...
function buildResultsHead(r){
  const n=r.questions?.length||0; el.resultsHead.innerHTML='';
  ['이름',...Array.from({length:n},(_,i)=>`Q${i+1}`),'점수','상태'].forEach(t=>{ const th=document.createElement('th'); th.textContent=t; el.resultsHead.appendChild(th); });
}
function buildResultsBody(r,arr){
  ...
  arr.forEach(s=>{
    ...
    const tdA=document.createElement('td'); tdA.textContent=s.alive===false?'out':'alive'; tr.appendChild(tdA);
    el.resultsBody.appendChild(tr);
  });
}
el.btnExportCSV.addEventListener('click', async ()=>{
  if(!roomId) return; const r=await fsGetRoom(roomId), arr=await fsGetResponses(roomId);
...
el.btnResetAll.addEventListener('click', async ()=>{
  if(!roomId) return; if(!confirm('모든 응답과 진행 상태를 초기화할까요?')) return;
...
el.tabs.forEach(t=>t.addEventListener('click',()=>{
  ...
  const name=t.dataset.tab; if(name==='build') el.tabBuild.classList.remove('hidden');
...
window.addEventListener('DOMContentLoaded', async ()=>{
  ...
  const pRoom=u.searchParams.get('room'); const isStudent=u.searchParams.get('student')==='1';
  const lastRoom=localStorage.getItem(KEYS.LAST_ROOM); const lastMode=localStorage.getItem(KEYS.LAST_MODE)||'teacher';

  // 모드 설정 우선순위: URL(student) > 저장값
  setMode(isStudent?'student':lastMode);

  // 룸 우선순위: URL(room) > 저장값
  const targetRoom = pRoom || lastRoom;
  if(targetRoom){ el.roomIdInput.value=targetRoom; await connectRoom(); }
