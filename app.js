...
let mcqSelected = null;       // 학생 객관식 선택값
...
const els = {
  ...
  // build
  panelBuild: $('#panelBuild'), quizTitle: $('#quizTitle'), questionCount: $('#questionCount'),
  ...
  // options
  panelOptions: $('#panelOptions'), chkAccept: $('#chkAccept'), chkReveal: $('#chkReveal'),
  chkBright: $('#chkBright'), timerSec: $('#timerSec'), btnSaveOptions: $('#btnSaveOptions'),
  ...
  // present
  panelPresent: $('#panelPresent'), btnStart: $('#btnStart'), btnPrev: $('#btnPrev'), btnNext: $('#btnNext'),
  btnEndAll: $('#btnEndAll'), leftSec: $('#leftSec'), pTitle: $('#pTitle'), pQ: $('#pQ'), pOpts: $('#pOpts'),
  presentWait: $('#presentWait'), statJoin: $('#statJoin'), statSubmit: $('#statSubmit'),
  statCorrect: $('#statCorrect'), statWrong: $('#statWrong'),

  // results
  panelResults: $('#panelResults'), btnExportCSV: $('#btnExportCSV'), btnResetAll: $('#btnResetAll'),
...
function collectBuilder(){
  ...
  const list = cards.map((card,idx)=>{
    ...
    if(type==='mcq'){
      ...
      return { type:'short', text, answerText: card.querySelector('.ansText').value.trim() };
    }
  }).filter(Boolean);
...
els.fileUploadTxt?.addEventListener('change', async (e)=>{
  ...
  for(const line of lines){
    ...
    if(cols.length===3 && /주관식/i.test(cols[1])){
      parsed.push({ type:'short', text:cols[0], answerText:cols[2] });
  ...
  if(!parsed.length){ alert('가져올 문제가 없습니다. 형식을 확인하세요.'); e.target.value=''; return; }
  ...
  alert(`총 ${parsed.length}문항을 불러왔습니다.`);
  e.target.value='';
});
els.btnDownloadTemplate?.addEventListener('click', ()=>{
  ...
  a.href=URL.createObjectURL(blob); a.download='quiz-template.csv'; a.click();
...
async function finishAll(){ 
  if(confirm('퀴즈를 종료할까요?')) await updateDoc(roomRef(roomId), { mode:'ended', accept:false }); 
}
...
function startTimer(sec){
  ...
  timerHandle=setInterval(()=>{
    ...
    if(remain<=0){
      stopTimer();
      updateDoc(roomRef(roomId), { accept:false }).catch(()=>{});
...
function renderRoom(r){
  // 옵션 탭 QR 노출
  const tabActive = $('.tab.active')===els.tabOptions;
  els.studentAccess?.classList.toggle('hide', !tabActive);

  // 타이머 자동 시작(프레젠트 진입시)
  if(r.mode==='active' && timerHandle==null){
    startTimer(r.timerSec||30);
  }
  // 프레젠테이션 테마
  ...
  if(idx>=0 && r.questions[idx]){
    ...
    if(q.type==='short'){ els.pOpts.innerHTML='<div class="muted">주관식 문제입니다.</div>'; }
  }else{
  ...
  if(MODE==='student'){
    ...
    if(r.mode!=='active' || !q){
      ...
      els.mcqBox&&(els.mcqBox.innerHTML=''); els.shortBox?.classList.add('hide');
...
function renderResponses(list){
  ...
  list.forEach(s=>{
    ...
    if(a!=null){ submitted++; if(a.correct) correct++; else wrong++; }
  });
  ...
  if(els.resultsTable){
    ...
    scored.forEach(({s,score})=>{
      ...
      (r.questions||[]).forEach((q,i)=>{
        ...
        if(a==null) td.textContent='-';
        else td.textContent= (q.type==='mcq') ? (typeof a.value==='number'? a.value+1 : '-') : (a.value??'-');
        tr.appendChild(td);
      });
  ...
  // 학생 개인 결과(종료 시)
  if(MODE==='student' && window.__room?.mode==='ended' && me.id){
    const mine = list.find(x=>x.id===me.id);
    if(!mine) return;
    const qList = window.__room.questions||[];
    const rows = qList.map((q,i)=>{
      const a=mine.answers?.[i];
      const sub = a==null? '-' : (q.type==='mcq' ? (typeof a.value==='number' ? a.value+1 : '-') : (a.value??'-'));
...
function buildStudentLink(){
  ...
  url.searchParams.set('role','student'); url.searchParams.set('room',roomId);
  els.studentLink.value=url.toString();

  if(window.QRCode && els.qrCanvas){
    try{ window.QRCode.toCanvas(els.qrCanvas, els.studentLink.value, { width:160, margin:1 }); }
...
[els.tabBuild,els.tabOptions,els.tabPresent,els.tabResults].forEach(btn=>{
  btn?.addEventListener('click', ()=>{
    ...
    if(btn===els.tabPresent && window.__room) startTimer(window.__room.timerSec||30);
  });
});
...
// URL 직접 진입 처리 (?role=student&room=class1)
(function fromURL(){
  const url=new URL(location.href);
