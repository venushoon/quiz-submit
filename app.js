...
const els={
  ...
  leftSec:$('#leftSec'),pTitle:$('#pTitle'),pQ:$('#pQ'),pOpts:$('#pOpts'),presentWait:$('#presentWait'),
...
function setMode(m){
  ...
  $('.tabs')?.classList.toggle('hide',!isAdmin);
  els.studentPanel?.classList.toggle('hide',isAdmin);
  // 옵션 탭 전용 영역은 기본 숨김
  els.studentAccess?.classList.add('hide');
  const st=roomId?`세션: ${roomId} · 온라인`:`세션: - · 오프라인`;
  els.roomStatus&&(els.roomStatus.textContent=st);
  els.studentTopInfo&&(els.studentTopInfo.textContent=st);
...
function signOut(){
  ...
  els.studentLink&&(els.studentLink.value=''); const ctx=els.qrCanvas?.getContext('2d'); ctx?.clearRect(0,0,els.qrCanvas.width,els.qrCanvas.height);
  saveLocal();
}
...
/* 탭 전환을 함수로(초기 화면 강제 설정용) */
function setActiveTab(whichBtn){
  [els.tabBuild,els.tabOptions,els.tabPresent,els.tabResults].forEach(b=>b?.classList.remove('active'));
  whichBtn?.classList.add('active');
  els.panelBuild?.classList.toggle('hide', whichBtn!==els.tabBuild);
  els.panelOptions?.classList.toggle('hide', whichBtn!==els.tabOptions);
  els.panelPresent?.classList.toggle('hide', whichBtn!==els.tabPresent);
  els.panelResults?.classList.toggle('hide', whichBtn!==els.tabResults);
  // 옵션 탭일 때만 학생접속 표시
  els.studentAccess?.classList.toggle('hide', whichBtn!==els.tabOptions);
}
...
function cardRow(no,q){
  ...
  wrap.innerHTML=`
    ...
    <div class="mcq ${q?.type==='short'?'hide':''}">
      <div class="row wrap gap">
        ${(q?.options||['','','','']).map((v,i)=>`<div class="row"><label class="badge">보기${i+1}</label><input class="opt input" data-no="${no}" data-idx="${i}" placeholder="보기${i+1}" value="${v||''}" /></div>`).join('')}
      </div>
      <div class="row wrap gap">
...
function collectBuilder(){
  ...
  const list=cards.map((c,idx)=>{
    ...
    const text=c.querySelector('.qtext').value.trim(); if(!text) return null;
    if(type==='mcq'){
...
els.fileUploadTxt?.addEventListener('change',async e=>{
  ...
  for(const line of lines){
    ...
    if(cols.length===3 && /주관식/i.test(cols[1])) parsed.push({type:'short',text:cols[0],answerText:cols[2]});
    else if(cols.length>=6){
      const text=cols[0]; const options=cols.slice(1,cols.length-1).filter(Boolean);
      const ans=Math.max(0,Math.min(options.length-1,(parseInt(cols.at(-1),10)||1)-1));
      parsed.push({type:'mcq',text,options,answerIndex:ans});
    }
  }
  ...
  els.quizTitle.value='업로드 퀴즈'; els.questionCount.value=parsed.length;
  e.target.value='';
});
...
function renderRoom(r){
  ...
  if(idx>=0 && r.questions[idx]){
    const q=r.questions[idx]; els.pQ.textContent=q.text; els.pOpts.innerHTML='';
  ...
  } else { els.pQ.textContent='-'; els.pOpts.innerHTML=''; }

  if(MODE==='student'){
    els.studentTopInfo&&(els.studentTopInfo.textContent=roomId?`세션: ${roomId} · 온라인`:`세션: - · 오프라인`);
    const q=r.questions?.[idx];
    if(r.mode!=='active'||!q){ els.badgeType&&(els.badgeType.textContent='대기'); els.sQText&&(els.sQText.textContent='대기 중입니다…'); els.mcqBox&&(els.mcqBox.innerHTML=''); els.shortBox?.classList.add('hide'); els.btnSubmitMCQ?.classList.add('hide'); return; }
    els.badgeType&&(els.badgeType.textContent=q.type==='mcq'?'객관식':'주관식'); els.sQText&&(els.sQText.textContent=q.text);
    if(q.type==='mcq'){
      els.mcqBox.innerHTML=''; mcqSelected=null;
      q.options.forEach((opt,i)=>{ const b=document.createElement('button'); b.className='optbtn'; b.textContent=`${i+1}. ${opt}`; b.disabled=!r.accept;
...
function buildStudentLink(){
  ...
  if(window.QRCode && els.qrCanvas){ try{ window.QRCode.toCanvas(els.qrCanvas, els.studentLink.value, {width:150,margin:1}); }catch(e){ console.warn(e);} }
}
...
els.btnLoadSample?.addEventListener('click',()=>{ const S=[{type:'mcq',text:'가장 큰 행성은?',options:['지구','목성','화성','금성'],answerIndex:1},{type:'short',text:'물의 끓는점(°C)은?',answerText:'100'},{type:'mcq',text:'태양계 별명은?',options:['Milky','Solar','Sunset','Lunar'],answerIndex:1},]; els.builder.innerHTML=''; S.forEach((q,i)=>els.builder.appendChild(cardRow(i+1,q))); els.quizTitle.value='샘플 퀴즈'; els.questionCount.value=S.length; });
els.btnSaveQuiz?.addEventListener('click',async()=>{ if(!roomId) return alert('세션에 먼저 접속하세요.'); const payload=collectBuilder(); if(!payload.questions.length) return alert('문항을 추가하세요.'); await setDoc(roomRef(roomId),{title:payload.title,questions:payload.questions},{merge:true}); alert('저장 완료!'); });

els.btnSaveOptions?.addEventListener('click',async()=>{ if(!roomId) return alert('세션에 먼저 접속하세요.'); const policy=$('#policyName')?.checked?'name':'device'; const bright=!!els.chkBright?.checked; const accept=!!els.chkAccept?.checked; const reveal=!!els.chkReveal?.checked; const tsec=Math.max(5,Math.min(600,parseInt(els.timerSec.value,10)||30)); await setDoc(roomRef(roomId),{policy,bright,accept,reveal,timerSec:tsec},{merge:true}); buildStudentLink(); alert('옵션 저장 완료!'); });
...
els.btnExportCSV?.addEventListener('click',async()=>{ const r=(await getDoc(roomRef(roomId))).data(); const snap=await getDocs(respCol(roomId)); const rows=[]; rows.push(['userId','name',...(r.questions||[]).map((_,i)=>`Q${i+1}`),'score'].join(',')); snap.forEach(d=>{ const s=d.data(); let score=0; const answers=(r.questions||[]).map((q,i)=>{ const a=s.answers?.[i]; if(a?.correct) score++; return q.type==='mcq'?(typeof a?.value==='number'?a.value+1:''):(a?.value??''); }); rows.push([d.id,`"${(s.name||'').replace(/"/g,'""')}"`,...answers,score].join(',')); }); const blob=new Blob([rows.join('\n')],{type:'text/csv'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`${r.title||roomId}-results.csv`; a.click(); URL.revokeObjectURL(a.href); });
els.btnResetAll?.addEventListener('click',async()=>{ if(!confirm('모든 응답/점수를 초기화할까요?')) return; await setDoc(roomRef(roomId),{mode:'idle',currentIndex:-1,accept:false,reveal:false},{merge:true}); const snap=await getDocs(respCol(roomId)); const ts=[]; snap.forEach(d=>ts.push(setDoc(doc(respCol(roomId),d.id),{answers:{},alive:true},{merge:true}))); await Promise.all(ts); alert('초기화 완료'); });
...
autoReconnect();
(function fromURL(){ const url=new URL(location.href); const role=url.searchParams.get('role'); const rid=url.searchParams.get('room'); if(role==='student'){ MODE='student'; setMode('student'); } if(rid){ els.roomId&&(els.roomId.value=rid); connect(); }})();
